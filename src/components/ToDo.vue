<template>
  <div class="to-do">
    ...
    <el-select v-model="selectedFilter" placeholder="Filter" size="large" style="width: 100%;" filterable>
      <el-option v-for="item in selectedList"
        :key="item.value"
        :label="item.label"
        :value="item.value">
      </el-option>
    ...
    <to-do-list
      ...
      :toDoList="filteredToDo"></to-do-list>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      searchToDo() {
        return this.toDoList.filter(item => item.title.toLowerCase().includes(this.search.toLowerCase()))
      },
    ...
    methods: {
      async getToDoList() {
        try {
          this.loading = true
          const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/todos?_limit=10`)
          this.toDoList = data
          this.setFavorites()
        } catch (error) {
      ...
      setFavorites() {
        ...
        const idFavoritesList = JSON.parse(localStorage.getItem('idFavoritesList'))
      ...
      isFavoritesToDo(item) {
        ...
        let idFavoritesList = JSON.parse(localStorage.getItem('idFavoritesList')) || []
        if (this.toDoList[idx].favorites) {
          ...
          const indexForIdFavoritesList = idFavoritesList.indexOf(this.toDoList[idx].id);
